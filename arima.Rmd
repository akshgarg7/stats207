...
ggsave(filename = "all_crops.png", width=10, height=6)
```
...
result <- fixed_model_pred(ts_train, ts_test)
title <- sprintf("ARIMA(1,1,0), fixed; MSE=%s",round(result$mse_test,2))
result$plot + ggtitle(title)
...
title <- sprintf("ARIMA(1,0,0)(0,1,0), rolling; MSE=%s", round(result$mse_test,2))
result$plot + ggtitle(title)

# One-step ahead forecasting with (1,1,0)
order=c(1,1,0)
result <- rolling_model_pred(ts_soy, soy_train, soy_test, order=order)
title <- sprintf("ARIMA(1,1,0), rolling; MSE=%s", round(result$mse_test,2))
result$plot + ggtitle(title)
...
summary(model)
order <- c(1,2,0)
result <- rolling_model_exog_pred(ts_soy, soy_train, soy_test, order=order, xreg=xreg)
title <- sprintf("ARIMA(1,2,0) with exog, rolling; MSE=%s", round(result$mse_test,2))
result$plot + ggtitle(title)

order <- c(1,0,2)
result <- rolling_model_exog_pred(ts_soy, soy_train, soy_test, order=order, xreg=xreg)
title <- sprintf("ARIMA(1,0,2) with exog, rolling; MSE=%s", round(result$mse_test,2))
result$plot + ggtitle(title)

# One-step ahead forecasting with 1-lagged exogenous variables (forecasted values)
xreg_train <- cbind(
  create_lag(res_barley_train, lag = 1),
  create_lag(res_wheat_train, lag = 1),
  create_lag(res_maize_train, lag = 1),
  create_lag(res_barley_train, lag = 2),
  create_lag(res_wheat_train, lag = 2),
  create_lag(res_maize_train, lag = 2)
)
xreg_test <- cbind(
...
order <- c(2,0,0)
result <- rolling_model_exog_pred(ts_soy, soy_train, soy_test, order=order, xreg=xreg)
title <- sprintf("ARIMA(2,0,0) with lagged exog, rolling; MSE=%s", round(result$mse_test,2))
result$plot + ggtitle(title)
...
xreg <- cbind(
  create_lag(res_barley, lag = 1),
  create_lag(res_wheat, lag = 1),
  create_lag(res_maize, lag = 1)
)
...
rolling_model_exog_pred <- function(ts, ts_train, ts_test, order, seasonal, xreg){
  ...
  res_test <- ts_test - fitted_vals_test
  fitted_train <- pred_lm(ts_train)
  res_train <- ts_train - fitted_vals_train
  ...
  p <- get_mse_and_plot(pred_train=pred_train + fitted_train, 
...
rolling_model_pred <- function(ts, ts_train, ts_test, order, seasonal){
  ...
  forecast_fun <- function(x,h){
    ...
    # Remove linear trend
    # linear_model <- lm(x~time(x))
    # res_x <- x - predict(linear_model)
    # m <- linear_model$coefficients[["time(x)"]]
    # b <- linear_model$coefficients[["(Intercept)"]]
    # next_t <-  time(x)[length(time(x))] + 1/12
    # next_val <- m*next_t + b
    ...
    rmses <<- c(rmses, rmse)
    forecast(model,h=h)
    # forecasted_vals$mean <- forecasted_vals$mean + next_val
...
get_mse_and_plot <- function(pred_train, pred_test, actual_train, actual_test){
  ...
  p <- autoplot(actual, series="actual") +
    ...
    geom_vline(xintercept=time(res_soy_test)[1], linetype="dashed", color="black") +
